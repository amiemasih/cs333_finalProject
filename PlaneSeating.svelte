<script>
  ...
  import { schemeSet3 } from 'd3-scale-chromatic'; // colorblind-friendly color scheme (i think??)
  ...
  // create color mapping using schemeSet3 (colorblind-friendly)
  let colorScheme = $derived.by(() => {
    const scheme = {};
    //update when fieldNames changes
    fieldNames.forEach((field, index) => {
      scheme[field] = schemeSet3[index % schemeSet3.length];
    });
    return scheme;
  });
    ...
    if (perCountryData) {
      ...
      for (const field of fieldNames) {
        ...
        for (let i = 0; i < seatsForField && seatIndex < 100; i++) {
          seatColors[seatIndex] = colorScheme[field];
...
<svelte:head>
  ...
  <div class="plane-container" bind:this={container}>
    ...
    <!-- left section - 20 seats - 2 columns × 10 rows) -->
    <div class="left-section">
      {#each Array(20) as _, seatIndex}
      <div 
        class="seat" 
    ...
    <!-- middle section - 30 seats - 3 columns × 10 rows) -->
    <div class="middle-section">
      {#each Array(30) as _, seatIndex}
        <div 
          class="seat" 
          style="background-color: {seatColors[seatIndex + 20]}"
          use:tooltip={`Seat ${seatIndex + 21}: ${seatFields[seatIndex + 20]}`}
        >
          <img src="https://raw.githubusercontent.com/amiemasih/cs333_finalProject/refs/heads/main/noun-person.svg" class="seat-icon" alt="person" />
        </div>
      {/each}
    ...
    <!-- right section - 20 seats - 2 columns × 10 rows) -->
    <div class="right-section">
      {#each Array(20) as _, seatIndex}
        <div 
          class="seat" 
          style="background-color: {seatColors[seatIndex + 50]}"
          use:tooltip={`Seat ${seatIndex + 51}: ${seatFields[seatIndex + 50]}`}
        >
          <img src="https://raw.githubusercontent.com/amiemasih/cs333_finalProject/refs/heads/main/noun-person.svg" class="seat-icon" alt="person" />
        </div>
      {/each}
  ...
  <div class="legend">
    {#each fieldNames as field}
      <div class="legend-item">
        <div class="legend-color" style="background-color: {colorScheme[field]}"></div>
        <span>{field}</span>
      </div>
    {/each}
  </div>
